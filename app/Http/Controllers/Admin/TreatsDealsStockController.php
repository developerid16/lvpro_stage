...
class TreatsDealsStockController extends Controller
{
    public function __construct()
    {
        ...
        $permission_prefix    = $this->permission_prefix    = 'treats-deals-stock';
        $this->layout_data    = [
            'permission_prefix' => $permission_prefix,
            'title'             => 'Treats & Deals Management Listing',
            'reward_base_url'   => url('admin/treats-deals-stock'),
    ...
    public function getLocationStock($id)
    {
        $reward = Reward::findOrFail($id);

        $locations = RewardLocation::where('reward_id', $reward->id)
            ->get()
            ->map(function ($loc) {
                return [
                    'id'            => $loc->location_id,
        ...
        return response()->json([
            'status' => true,
            'data'   => $locations,
            'hide_catalogue' => $reward->hide_catalogue,
            'hide_cat_time' => $reward->hide_cat_time
        ]);
    }

    public function locationStockAdjustment(Request $request)
    {
        $request->validate([
            'reward_id' => 'required|integer|exists:rewards,id',
            'locations' => 'required|array',
            'locations.*.location_id' => 'required|integer',
            'locations.*.qty' => 'required|integer|min:0',
        ]);
        ...
        try {
            ...
            /*
            |--------------------------------------------------------------------------
            | 1️⃣ Check RewardUpdateRequest
            |--------------------------------------------------------------------------
            */

            $updateRequest = RewardUpdateRequest::where('reward_id', $reward->id)
                ->where('type', '1') // if you use type
                ->first();

            if ($updateRequest) {

                // If exists → force pending
                ...
                // If not exists → create new from reward
                $data = $reward->toArray();
                unset($data['id']);
                ...
                $updateRequest = RewardUpdateRequest::create($data);
            }


            /*
            |--------------------------------------------------------------------------
            | 2️⃣ Store Location Updates
            |--------------------------------------------------------------------------
            */

            foreach ($request->locations as $loc) {

                $existingLocation = RewardLocationUpdate::where('reward_id', $reward->id)
                    ->where('location_id', $loc['location_id'])
                    ->first();

                $baseQty = $existingLocation
                    ? $existingLocation->inventory_qty
                    : 0;
